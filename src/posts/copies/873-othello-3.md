---
id: 1041
order: 0959
NSC: "873"
filename: 873-othello-3
copy_URL: /873-othello-3/
title_id: 22
title: |
  Othello
titleImage: othello.png
title_hyphen: othello
titleURL: /othello/
edition_URL: 
issue: FALSE
issue_id: 66
issue_notes: |
  [The title-page states this is "The fourth Edition" [DEEP]; "In this issue there is a comma after 'Othello' in line 2 of title" [ESTC]]
total_extant_copies: 181
title_fragment: 
isFragment: FALSE
apocryphal: FALSE
edition_id: 57
edition_number: ["3"]
edition_format: ["4°"]
STC_Wing: ["S2939"]
ESTC: ["R17082"]
year: 1655
start_date: 1655
end_date: 1655
yearCount: 1655
DEEP: 696
extant_ed_copies: 26
ed_fragment: 
location_id: 21
location_name: ["British Library"]
shelfmark: |
  S341.1655
isLocationVerified: TRUE
isFacsimileAvailable: FALSE
digital_facsimile_URL: 
summary: |
  Inlaid, cropped at head.
provenance_name: ["Adler, Elkan Nathan"]
prov_info: |
  Sir William Bunbury? - Sir Henry Bunbury - Payne & Foss - Devonshire copy (with the latter's initialed collation, 1827). Sir Thomas Hanmer's ownership is disputed by Freeman and Freeman.
height: 
width: 
marginalia: 
condition: 
binding: 
binder: 
---

